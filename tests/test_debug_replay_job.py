...
from frigate.application.jobs.debug_replay import (
    DebugReplayJob,
...
from frigate.application.jobs.export import JobStatePublisher
from frigate.application.jobs.manager import _completed_jobs, _current_jobs
...
class TestStartDebugReplayJob(unittest.TestCase):
    ...
    def test_rejects_when_no_recordings(self) -> None:
        ...
        with patch("frigate.application.jobs.debug_replay.query_recordings", return_value=empty_qs):
            with self.assertRaises(ValueError):
                start_debug_replay_job(
                    source=RecordingDebugReplaySource(
                        source_camera="front",
    ...
    def test_returns_job_id_and_marks_session_starting(self) -> None:
        ...
        with (
            patch(
                "frigate.application.jobs.debug_replay.query_recordings",
                return_value=self.recordings_qs,
            ),
            patch(
                "frigate.application.jobs.debug_replay.run_ffmpeg_with_progress",
...
class TestRunnerHappyPath(unittest.TestCase):
    ...
    def test_progress_callback_updates_job_percent(self) -> None:
        ...
        from frigate.application.jobs.manager import get_current_job
