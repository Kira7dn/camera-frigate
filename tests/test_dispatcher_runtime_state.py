...
def _build_dispatcher(cameras: dict[str, MagicMock]) -> Dispatcher:
    ...
    with (
        patch("frigate.infrastructure.comms.dispatcher.CameraActivityManager"),
        patch("frigate.infrastructure.comms.dispatcher.AudioActivityManager"),
    ):
...
class TestHandlersPersistViaSet(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        with open(self.config_path, "w") as f:
            f.write("")
        self._patcher = patch(
            "frigate.infrastructure.comms.runtime_state.find_config_file",
