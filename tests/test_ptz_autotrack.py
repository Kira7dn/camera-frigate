...
from frigate.domain.ptz.autotrack import PtzAutoTracker
...
def _make_tracker(autotracking_enabled: bool = True) -> PtzAutoTracker:
    ...
    tracker.intercept = {}
    tracker.zoom_time = {}
    tracker.config_patch_sink = None
    return tracker
...
class TestExternalCalibrationPatch(unittest.TestCase):
    def test_simulator_samples_use_native_regression_and_patch_sink(self) -> None:
        tracker = _make_tracker()
        patches = []
        tracker.config_patch_sink = lambda camera, patch: patches.append(
            (camera, patch)
        )

        accepted = tracker.calibrate_from_samples(
            CAMERA,
            [
                {"pan": 0.0, "tilt": 0.0, "duration": 0.2},
                {"pan": 0.5, "tilt": 0.0, "duration": 0.45},
                {"pan": 1.0, "tilt": 0.0, "duration": 0.7},
            ],
            min_zoom=0.1,
            max_zoom=0.9,
            zoom_time=0.4,
        ...
        self.assertEqual(patches[0][0], CAMERA)
        key = f"cameras.{CAMERA}.onvif.autotracking.movement_weights"
        self.assertEqual(set(patches[0][1]), {key})
