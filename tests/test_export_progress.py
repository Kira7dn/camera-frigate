...
from frigate.application.jobs.export import (
    PROGRESS_BROADCAST_MIN_INTERVAL,
    ExportJob,
    ExportJobManager,
)
from frigate.domain.record.export import PlaybackSourceEnum, RecordingExporter
...
class TestBroadcastAggregation(unittest.TestCase):
    ...
    def test_throttle_allows_broadcast_after_interval(self) -> None:
        ...
        with patch("frigate.application.jobs.export.time.monotonic") as mock_mono:
...
class TestSaveThumbnailFromPreviewFrames(unittest.TestCase):
    ...
    def test_short_export_falls_back_to_prior_preview_frame(self) -> None:
        ...
        with (
            patch(
                "frigate.domain.record.export.CACHE_DIR", os.path.join(self.tmp_root, "cache")
            ),
            patch(
                "frigate.domain.record.export.CLIPS_DIR", os.path.join(self.tmp_root, "clips")
            ),
        ):
...
class TestSchedulesCleanup(unittest.TestCase):
    def test_schedule_job_cleanup_removes_after_delay(self) -> None:
        ...
        with patch("frigate.application.jobs.export.threading.Timer") as mock_timer:
...
class TestChapterMetadataInProgressReview(unittest.TestCase):
    ...
    def test_in_progress_review_does_not_crash_and_clamps_to_last_recording(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "frigate.domain.record.export.ReviewSegment.select",
