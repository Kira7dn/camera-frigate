...
_MOCKED_MODULES = [
    "frigate.infrastructure.comms.inter_process",
    "frigate.infrastructure.comms.detections_updater",
    "frigate.infrastructure.comms.recordings_updater",
    "frigate.infrastructure.config.camera.updater",
]
...
from frigate.domain.record.maintainer import RecordingMaintainer  # noqa: E402
    ...
    async def test_move_files_survives_bad_filename(self):
        ...
        with patch("os.listdir", return_value=files):
            with patch("os.path.isfile", return_value=True):
                with patch(
                    "frigate.domain.record.maintainer.psutil.process_iter", return_value=[]
                ):
                    with patch("frigate.domain.record.maintainer.logger.warning") as warn:
