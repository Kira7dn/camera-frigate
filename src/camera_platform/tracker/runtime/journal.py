...
class EdgeJournal:
    ...
    def ack(self, sequence: int, event_id: str, node_epoch: str | None = None) -> bool:
        row = self._db.execute(
            "SELECT event_id, acked, payload FROM journal WHERE sequence=?", (sequence,)
        ).fetchone()
        if row is None or row["event_id"] != event_id:
            return False
        if (
            node_epoch is not None
            and TrackerUpdate.from_json(row["payload"]).node_epoch != node_epoch
        ):
            return False
        if row["acked"]:
            return True
    ...
    def recover_active(self, code: str = "process_restart") -> tuple[TrackerUpdate, ...]:
        """Append typed END entries for active state reconstructed from real journal."""
        active: dict[tuple[str, str, str, str], TrackerUpdate] = {}
        rows = self._db.execute(
            "SELECT payload FROM journal ORDER BY sequence"
        ).fetchall()
        for row in rows:
            update = TrackerUpdate.from_json(row["payload"])
            key = (
                update.node_id,
                update.camera_id,
                update.stream_epoch,
                ...
                self.append(
                    replace(
                        ...
                        failure=LifecycleFailure(
                            code,
                            "tracker process restarted with active producer state",
                            retryable=False,
                        ),
                    )
                )
