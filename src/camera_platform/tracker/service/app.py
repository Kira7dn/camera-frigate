...
from camera_platform.tracker.config.fingerprint import tracker_config_fingerprint
from camera_platform.tracker.runtime.node import TrackerNodeRuntime
...
async def _run(args: argparse.Namespace) -> None:
    ...
    config = FrigateConfig.parse(config_text, is_json=False, install=True)
    manager = mp.Manager()
            ...
            if isinstance(samples, list):
                try:
                    accepted = runtime.ptz.ptz_autotracker.calibrate_from_samples(
                        ...
                        [
                            {
                                "pan": float(sample["pan"]),
                                "tilt": float(sample["tilt"]),
                                "duration": float(sample["duration"]),
                            }
                            for sample in samples
                        ],
                        min_zoom=float(payload.get("min_zoom", 0)),
                        max_zoom=float(payload.get("max_zoom", 1)),
                        zoom_time=float(payload.get("zoom_time", 0)),
                    )
