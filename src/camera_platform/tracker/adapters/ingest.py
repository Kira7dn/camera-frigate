...
class TrackerHostIngest:
    ...
    def seed_sequence(self, node_id: str, node_epoch: str, sequence: int) -> None:
        ...
        self._last_sequences[node_id] = max(
            self._last_sequences.get(node_id, 0), sequence
        )
    ...
    def accept(self, update: TrackerUpdate) -> IngestAck:
        ...
        if (
            current_node_epoch is not None
            and current_node_epoch != update.node_epoch
            and any(key[0] == update.node_id for key in self._active)
        ):
