...
class RecognitionGrpcClient:
    ...
    async def configure(self, config_json: str) -> str:
        ...
        stub = self._stub
        if stub is None:
            raise RuntimeError("recognition gRPC stub is not connected")
        config_hash = hashlib.sha256(config_json.encode()).hexdigest()
        response = await stub.Configure(
            pb.ConfigureRequest(
    ...
    async def submit(self, job: RecognitionJob) -> JobReceipt:
        ...
        try:
            stream = self._stream
            if stream is None:
                raise RuntimeError("recognition gRPC stream is not connected")
            async with self._write_lock:
                await stream.write(envelope_from_job(job))
    ...
    async def manage_face_library(
        ...
        if stub is None:
            raise RuntimeError("recognition gRPC stub is not connected")
        return await stub.ManageFaceLibrary(
