...
class TrackerNodeRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        if set(config.tracker) != {node_id} or set(config.cameras) != set(node.cameras):
        ...
        self.stop_event = stop_event
        self._publish: Callable[[TrackerUpdate], None] | None = None
        ...
        edge_models = CAMERA_RUNTIME_MODELS + CAMERA_HISTORY_MODELS
        self.edge_database.bind(edge_models)
        self.edge_database.create_tables(edge_models, safe=True)
    ...
    def set_publisher(self, publish: Callable[[TrackerUpdate], None]) -> None:
        """Attach the live transport before camera workers start."""
        if self.processors:
            raise RuntimeError("tracker publisher must be set before runtime start")
