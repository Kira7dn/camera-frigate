...
from ..domain.contracts import TrackerUpdate
from ..runtime.evidence import EvidenceRing, EvidenceUnavailableError
from ..runtime.journal import EdgeJournal
from ..runtime.media import MediaAuthority
...
class TrackerGrpcService(pb_grpc.TrackerServiceServicer):
    def __init__(
        ...
        *,
        ...
        terminal_state: Callable[[], dict[str, int]] | None = None,
    ) -> None:
        ...
        self._subscribers: set[tuple[asyncio.AbstractEventLoop, asyncio.Queue[TrackerUpdate]]] = set()

    def publish_update(self, update: TrackerUpdate) -> None:
        """Publish a durable tracker update to connected main-runtime clients."""
        with self._subscriber_lock:
            subscribers = tuple(self._subscribers)
        for loop, updates in subscribers:
            loop.call_soon_threadsafe(self._offer_update, updates, update)
    ...
    def _offer_update(
        ...
        try:
            ...
            # The journal remains authoritative; the next client heartbeat
            # replays anything that could not fit in this live queue.
            return
    ...
    async def Connect(self, request_iterator, context):
        ...
        request_task = asyncio.create_task(request_iterator.__anext__())
            ...
            while True:
                ...
                if request_task in done:
                    ...
                    if request == "hello":
                        ...
                        elif not self.journal.ack(
                            ack.journal_sequence, ack.event_id, ack.node_epoch
                        ):
                            yield pb.EdgeEnvelope(
                                failure=pb.LifecycleFailure(
                        ...
                        for update in self.journal.replay(last_sent):
                            yield pb.EdgeEnvelope(update=update_to_proto(update))
                            last_sent = update.journal_sequence
                        yield pb.EdgeEnvelope(health=self._node_health())
    ...
    def _node_health(self) -> pb.NodeHealth:
        state = self.terminal_state() if self.terminal_state is not None else {}
        return pb.NodeHealth(
            node_id=self.node_id,
            node_epoch=self.node_epoch,
            ready=True,
            degraded=self.degraded,
            pending_ack=state.get("pending_ack", self.journal.pending_count),
            spool_bytes=state.get("spool_bytes", self.journal.logical_bytes),
            pinned_evidence=state.get(
                "pinned_evidence",
                sum(ring.pinned_count for ring in self.evidence.values()),
            ),
            active_lifecycles=state.get("active", 0),
        )
    ...
    async def Configure(self, request, context):
        ...
        if not isinstance(payload, dict) or payload.get("node_id") != self.node_id:
            await context.abort(grpc.StatusCode.INVALID_ARGUMENT, "config_node_mismatch")
        ...
        if digest != self.config_hash:
            await context.abort(
                ...
                "config_revision_requires_topology_restart",
            )
    ...
    async def GetCapabilities(self, request, context):
        ...
        response.health.CopyFrom(self._node_health())
        return response
