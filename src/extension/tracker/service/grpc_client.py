...
from ..adapters.ingest import TrackerHostIngest
...
class TrackerGrpcClient:
    def __init__(
        ...
        *,
        ...
        prepare: Callable[[TrackerUpdate], Awaitable[TrackerUpdate]] | None = None,
    ) -> None:
    ...
    async def connect(
        ...
        *,
        ...
        if config_json or config_hash:
            if not config_json or not config_hash:
                raise ValueError("tracker config_json and config_hash are required together")
            configured = await self._stub.Configure(
                pb.ConfigureRequest(
            ...
            if configured.config_hash != config_hash:
                raise RuntimeError("tracker_config_ack_mismatch")
        ...
        try:
            async for envelope in stream:
                ...
                if response == "hello":
                    ...
                    update = update_from_proto(envelope.update)
                    if self._prepare is not None:
                        update = await self._prepare(update)
                    # Canonical Event/SQLite acceptance may wait on the existing
                    # EventProcessor thread. Never block the asyncio gRPC loop.
                    ack = await asyncio.to_thread(self.ingest.accept, update)
                    await self._outgoing.put(
                        pb.MainEnvelope(
                            ack=pb.JournalAck(
                                node_id=ack.node_id,
    ...
    async def capabilities(self) -> pb.CapabilitiesResponse:
        ...
        stub = self._stub
        if stub is None:
            raise RuntimeError("tracker gRPC stub is not connected")
        return await stub.GetCapabilities(
    ...
    async def stream_media(
        ...
        output = bytearray()
        async for chunk in stub.StreamMedia(
    ...
    async def control_camera(
        self, camera_id: str, operation: int, payload: dict[str, object]
    ) -> pb.ControlResponse:
        if self._stub is None or not self.healthy:
            raise RuntimeError("tracker_edge_unavailable")
        return await self._stub.ControlCamera(
            pb.ControlRequest(
                command_id=uuid.uuid4().hex,
                camera_id=camera_id,
                operation=operation,
                payload_json=json.dumps(payload, sort_keys=True),
                expected_node_epoch=self.node_epoch,
            ),
            timeout=self.deadline,
        )
