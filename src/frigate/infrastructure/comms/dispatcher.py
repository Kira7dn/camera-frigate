...
from frigate.domain.camera.activity_manager import AudioActivityManager, CameraActivityManager
from frigate.infrastructure.comms.base_communicator import Communicator
from frigate.infrastructure.comms.runtime_state import RuntimeStatePersistence
from frigate.infrastructure.config import BirdseyeModeEnum, FrigateConfig
from frigate.infrastructure.config.camera.updater import (
    CameraConfigUpdateEnum,
    CameraConfigUpdatePublisher,
    CameraConfigUpdateTopic,
)
from frigate.infrastructure.config.config import RuntimeFilterConfig, RuntimeMotionConfig
...
from frigate.domain.ptz.onvif import OnvifCommandEnum, OnvifController
...
class Dispatcher:
    ...
    def __init__(
        ...
        edge_control: Callable[[str, str, dict[str, object]], bool] | None = None,
        edge_cameras: frozenset[str] | None = None,
    ) -> None:
        self.config = config
        ...
        self.edge_cameras = edge_cameras or frozenset()
    ...
    def _on_ptz_autotracker_command(self, camera_name: str, payload: str) -> None:
        ...
        if camera_name in self.edge_cameras and payload in ("ON", "OFF"):
            if self.edge_control is None or not self.edge_control(
                camera_name,
                "enable" if payload == "ON" else "disable",
                {"target": "autotracking"},
            ):
                logger.error(
                    "Tracker rejected autotracking command for %s", camera_name
                )
                return
    ...
    def _on_ptz_command(self, camera_name: str, payload: str | bytes) -> None:
        ...
        try:
            ...
            if camera_name in self.edge_cameras:
                if self.edge_control is None:
                    logger.error("Tracker control unavailable for %s", camera_name)
                    return
                operation = "preset" if command is OnvifCommandEnum.preset else "manual_ptz"
                control_payload = (
                    {"preset": param}
                    if operation == "preset"
