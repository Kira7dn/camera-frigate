...
from frigate.infrastructure.config import FrigateConfig, ZoomingModeEnum
from frigate.infrastructure.config.config import RuntimeMotionConfig
...
class CameraState:
    ...
    def get_current_frame(self, draw_options: dict[str, Any] = {}) -> np.ndarray:
        ...
        if draw_options.get("mask"):
            motion_config = cast(RuntimeMotionConfig, self.camera_config.motion)
            if motion_config.rasterized_mask is None:
                return frame_copy
            mask_overlay = np.where(motion_config.rasterized_mask == [0])
        ...
        for id in new_ids:
            ...
            for c in self.callbacks["start"]:
                c(
                    ...
                    new_obj,
                    frame_name,
                    new_obj.obj_data["frame_time"] == frame_time,
                )
            ...
            if autotracker_update or significant_update:
                for c in self.callbacks["autotrack"]:
                    c(
                        ...
                        updated_obj,
                        frame_name,
                        updated_obj.obj_data["frame_time"] == frame_time,
                    )
        ...
        for id in removed_ids:
            ...
            if "end_time" not in removed_obj.obj_data:
                ...
                for c in self.callbacks["end"]:
                    c(self.name, removed_obj, frame_name, False)
