...
from frigate.infrastructure.config import CameraConfig, FrigateConfig, ZoomingModeEnum
...
class PtzAutoTracker:
    ...
    def _write_config(self, camera):
        ...
        config_patch_sink = getattr(self, "config_patch_sink", None)
        if config_patch_sink is not None:
            config_patch_sink(camera, patch)
            return
        update_yaml_file_bulk(find_config_file(), patch)

    def request_calibration(self, camera: str):
        """Run the native calibration loop on the ONVIF event loop."""
        return asyncio.run_coroutine_threadsafe(
            self._calibrate_camera(camera), self.onvif.loop
        )
    ...
    def calibrate_from_samples(
        ...
        *,
        ...
        """Feed simulator/replay measurements into the native regression path."""
        self.move_metrics[camera] = [
            {
                "pan": sample["pan"],
                "tilt": sample["tilt"],
                "start_timestamp": 0.0,
                "end_timestamp": sample["duration"],
            }
            for sample in samples
        ]
        self.ptz_metrics[camera].min_zoom.value = min_zoom
        self.ptz_metrics[camera].max_zoom.value = max_zoom
        self.zoom_time[camera] = zoom_time
        return bool(self._calculate_move_coefficients(camera, True))
