...
async def _edge_media_response(
    ...
    runtime = getattr(request.app, "tracker_maintainer", None)
    if runtime is None:
        return None
    try:
        resolved = resolve_event_media(
            event_id, media_type, request.headers.get("range")
        )
    ...
    try:
        data = await runtime.fetch_media(
            manifest.node_id,
            manifest.media_id,
        ...
        logger.exception("Edge media unavailable: %s", manifest.media_id)
        return JSONResponse(
            content={"success": False, "message": "Edge media unavailable"},
            status_code=503,
        )
    headers = {
        "Accept-Ranges": "bytes",
    ...
    if resolved.length is not None:
        ...
        headers["Content-Range"] = (
            f"bytes {resolved.offset}-{resolved.offset + len(data) - 1}/"
            f"{manifest.byte_size}"
        )
    return Response(
        data, media_type=content_type, headers=headers, status_code=status_code
    )
...
async def recording_clip(
    ...
    config: FrigateConfig = request.app.frigate_config
    try:
        prepared = prepare_recording_clip(config, camera_name, start_ts, end_ts)
    except ValueError:
        return JSONResponse(
            content={"success": False, "message": "Filename exceeded max length of 1000"},
            status_code=403,
        )
    if prepared is None:
        return JSONResponse(
    ...
    playlist, ffmpeg_cmd = prepared

    return StreamingResponse(
        run_download(ffmpeg_cmd, os.fspath(playlist)),
...
async def event_snapshot(
    ...
    try:
        ...
        if edge_response is not None:
            if params.download:
                edge_response.headers["Content-Disposition"] = (
...
async def event_snapshot_clean(request: Request, event_id: str, download: bool = False):
    ...
    edge_response = await _edge_media_response(
        request, event_id, "snapshot", "image/webp"
    )
    if edge_response is not None:
        if download:
            edge_response.headers["Content-Disposition"] = (
...
async def event_clip(
    ...
    edge_response = await _edge_media_response(
        request, event_id, "clip", "video/mp4"
    )
    if edge_response is not None:
        return edge_response
