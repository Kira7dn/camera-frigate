...
from frigate.infrastructure.config import FrigateConfig, GenAIConfig, GenAIProviderEnum
...
from frigate.application.genai import PROVIDERS, load_providers
from frigate.application.jobs.media_sync import (
    get_current_media_sync_job,
    get_media_sync_job_by_id,
    start_media_sync_job,
)
from frigate.models import Event, Timeline
from frigate.application.stats.prometheus import get_metrics, update_metrics
...
def metrics(request: Request):
    ...
    # This sync endpoint runs in Starlette's worker pool. Explicitly own and
    # close the thread-local Peewee read connection; otherwise Windows keeps
    # the SQLite file locked after the request has completed.
    with Event._meta.database.connection_context():
        event_counts: list[dict[str, Any]] = list(
    ...
    try:
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Your configuration is invalid.\nSee the official documentation at docs.frigate.domain.video.\n\n"
                    + "\n".join(error_message),
                }
            ),
            status_code=400,
