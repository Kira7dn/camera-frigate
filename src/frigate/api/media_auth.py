...
def _resolve_export(
    ...
    filename = parts[1]
    # EXPORT_DIR is a container/runtime POSIX contract even when unit tests run
    # on Windows. Native os.path.join would insert a backslash and miss the row.
    full_path = posixpath.join(EXPORT_DIR, filename)
    try:
