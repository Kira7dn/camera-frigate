...
async def notification_snapshot(
    ...
    if snapshot is None:
        manifest = resolve_media_id(artifact_id)
        runtime = getattr(request.app, "tracker_maintainer", None)
        if manifest is not None and runtime is not None:
            try:
                snapshot = await runtime.fetch_media(
                    manifest.node_id, manifest.media_id
                )
            except (grpc.RpcError, RuntimeError, TimeoutError):
                logger.exception(
                    "Edge notification media unavailable: %s", artifact_id
                )
                return JSONResponse(
                    {"success": False, "message": "Edge media unavailable"},
                    status_code=503,
                )
    if snapshot is None:
        return JSONResponse(
