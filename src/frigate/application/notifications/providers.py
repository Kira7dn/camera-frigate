...
class TelegramProvider:
    ...
    def __init__(self, signer: NotificationMediaSigner | None = None) -> None:
        self.signer = signer or NotificationMediaSigner()
    ...
    async def deliver(
        ...
        try:
            if snapshot:
            ...
            elif artifact_ref and public_base_url:
                response = await client.post(
                    f"{base_url}/sendPhoto",
                    data={
                ...
                return DeliveryResult(False, True, "Artifact media URL is unavailable")
            else:
                response = await client.post(
