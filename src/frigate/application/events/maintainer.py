...
class EventProcessor(threading.Thread):
    def __init__(
        ...
        tracker_commit_queue: Any | None = None,
    ):
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            receipt_id = str(update[5]) if len(update) > 5 and update[5] else ""
            try:
                self._process_event_update(update[:5])
            except Exception as error:
                if not receipt_id or self.tracker_commit_queue is None:
                    raise
                logger.exception("Tracker Event transaction failed receipt=%s", receipt_id)
                self.tracker_commit_queue.put((receipt_id, False, str(error)))
            else:
                if receipt_id and self.tracker_commit_queue is not None:
                    self.tracker_commit_queue.put((receipt_id, True, ""))
    ...
    def _process_event_update(self, update: Any) -> None:
