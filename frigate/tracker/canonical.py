...
from .host import TrackerIngestError
...
class TrackerCanonicalStore:
    ...
    def last_sequence(self, node_id: str, node_epoch: str) -> int:
        row = (
            TrackerJournalEntry.select(TrackerJournalEntry.journal_sequence)
            .where(
                (TrackerJournalEntry.node_id == node_id)
                & (TrackerJournalEntry.node_epoch == node_epoch)
            )
